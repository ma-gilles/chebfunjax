...
Translation of approx/AAAApprox.m by Nick Trefethen (December
...
def run():
    ...
    fig.set_facecolor("white")
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "AAAApprox_03.png"), size=(600, 270))
    plt.close(fig)
    ...
    fig.set_facecolor("white")
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "AAAApprox_05.png"), size=(600, 270))
    plt.close(fig)

    # AAA of |x|
    r, *_ = _aaa_on_interval(lambda z: jnp.abs(z))
    _ezplot(lambda x: abs(x) - np.real(r(np.asarray([x]))[0]),
            (-1, 1), "AAAApprox_06.png", ylim=(-5e-14, 5e-14))
    ...
    fig.set_facecolor("white")
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "AAAApprox_07.png"), size=(600, 270))
    plt.close(fig)
