...
Translation of calc/DeltaDerivs.m by Nick Trefethen: a sine wave
...
def run():
    ...
    _plot(f, "b", "f:  a sine wave plus a sequence of delta impulses",
          "DeltaDerivs_01", deltas=f.deltas)
    ...
    g = f.cumsum()
    _plot(g, "r", "The integral of f", "DeltaDerivs_02")
    h = g.cumsum()
    _plot(h, "g", "The second integral of f", "DeltaDerivs_03")
    q = h.cumsum()
    _plot(q, "orange", "The third integral of f", "DeltaDerivs_04")

    f2 = q.diff().diff().diff()
    _plot(f2, "b", "f again, obtained via a third derivative",
          "DeltaDerivs_05", deltas=getattr(f2, "deltas", ()))
    return True
