...
Translation of calc/SurfaceRevolution.m by Georges Klein: surfaces
...
def run():
    ...
    fig.set_facecolor("white")
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "SurfaceRevolution_01.png"))
    plt.close(fig)
    _cylinder_surf(f, (-1, 1), "SurfaceRevolution_02")
    f = cj.chebfun(lambda x: -3 * x + 3)
    _cylinder_surf(f, (-1, 1), "SurfaceRevolution_03")
    f = cj.chebfun(lambda x: 2.8 * jnp.sin(0.2 * x - 0.1) + 6.3,
                   domain=[-5, 35])
    _cylinder_surf(f, (-5, 35), "SurfaceRevolution_04",
                   orient="x")

    x = cj.chebfun(lambda t: t, domain=[0, 2 * np.pi])
    f = cj.chebfun(lambda t: jnp.sqrt(4 + 2 * jnp.sin(2 * t)),
                   domain=[0, 2 * np.pi])
    _cylinder_surf(f, (0, 2 * np.pi), "SurfaceRevolution_05",
                   zscale=2 * np.pi, zlim=(0, 8))
    ...
    print("zG =")
    print(f"   {zG:.15f}")
    # hold on, alpha(.1), plot3(0,0,zG,'.r','MarkerSize',40), zlim([0 8])
    _cylinder_surf(f, (0, 2 * np.pi), "SurfaceRevolution_06",
                   zscale=2 * np.pi, zlim=(0, 8),
                   extra=lambda ax: ax.plot([0], [0], [zG], '.r', ms=20,
                                            zorder=10))
    ...
    theta = np.arange(13) / 12 * 2 * np.pi
    r4 = float(fls(jnp.asarray(0.4))) / 2

    def _dots(ax):
        ax.plot(np.cos(theta), np.sin(theta), .01 * np.ones(13), 'g.')
        ax.plot(r4 * np.cos(theta), r4 * np.sin(theta), .17 * np.ones(13), 'r.')
        ax.plot(r4 * np.cos(theta), r4 * np.sin(theta), -.17 * np.ones(13), 'y.')
        ax.set_axis_off()
    _cylinder_surf(fls, (-1, 1), "SurfaceRevolution_07",
                   zscale=0.25, view=(10, 0), extra=_dots)
    return True
