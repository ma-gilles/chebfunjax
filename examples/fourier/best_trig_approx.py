...
Translation of fourier/BestTrigApprox.m by Mohsin Javed and Nick
...
def _save(fig, stem=None):
    FIG[0] += 1
    fig.set_facecolor("white")
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, f"BestTrigApprox_{FIG[0]:02d}.png"))
    plt.close(fig)
...
def run():
    ...
    f4 = cj.chebfun(lambda x: 1 / (1.01 - jnp.cos(x)),
                    domain=[-np.pi, np.pi], trig=True)
