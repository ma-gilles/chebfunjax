...
Translation of roots/BesselRoots.m by Nick Trefethen
...
def run():
    ...
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "BesselRoots_01.png"), size=(600, 270))

    r = np.asarray(J0.roots())
    ax.plot(r, np.asarray(J0(r)), '.r', ms=9)
    _savefig(fig, os.path.join(_IMG, "BesselRoots_02.png"), size=(600, 270))
    plt.close(fig)
