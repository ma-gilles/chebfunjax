...
Translation of approx2/BumpFunction.m: a sum of 100 randomly
...
def run():
    ...
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "BumpFunction_01.png"))
    plt.close(fig)
    ...
    fig.set_facecolor("white")
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "BumpFunction_02.png"))
    plt.close(fig)
    ...
    fig.set_facecolor("white")
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "BumpFunction_03.png"))
    plt.close(fig)
    ...
    fig.set_facecolor("white")
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "BumpFunction_04.png"))
    plt.close(fig)

    # [m,X] = max2(f); plot(f), hold on, plot3(X(1),X(2),m,'k.'), zlim([0,5])
    m, X0 = f.max2()
    X0 = np.ravel(np.asarray(X0))
    gx = np.linspace(-1, 1, 160)
    GX, GY = np.meshgrid(gx, gx)
    fig = plt.figure(figsize=(8.0, 5.6))
    ax = fig.add_subplot(projection="3d")
    ax.plot_surface(GX, GY, np.asarray(f(GX, GY)),
                    cmap="viridis", rstride=2, cstride=2, linewidth=0)
    ax.plot([float(X0[0])], [float(X0[1])], [float(m)], 'k.', ms=18)
    ax.set_zlim(0, 5)
    ax.set_title("Global maximum of f")
    fig.set_facecolor("white")
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "BumpFunction_05.png"))
    plt.close(fig)
