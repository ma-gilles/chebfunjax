...
def run():
    ...
    # For harmonic oscillator: P = alpha*E where alpha = 1/(2*omega^3) = 1/16
    alpha_numerical = -(E0_vals[-1] - E0_vals[0]) / (E_vals[-1] - E_vals[0])
    print(f"  Numerical alpha (linear polarizability) ≈ {alpha_numerical:.4f}")
    # Exact: alpha = 1/(2 omega^3) for V = omega^2 x^2/2 with m=1
    # Our V = 2x^2 => omega^2/2 = 2 => omega = 2, alpha = 1/(2*8) = 1/16 = 0.0625
    ...
    assert err_alpha < 0.05, f"Polarizability error too large: {err_alpha}"
