...
Translation of ode-random/GBM.m by Nick Trefethen (May 2017):
...
def run():
    ...
    fs = [randnfun(LAMBDA, DOM, big=True, key=jax.random.PRNGKey(k))
          for k in range(5)]
    _panel(0.0, fs, "zero drift", "GBM_01.png")
    print("zero drift done", flush=True)
    _panel(0.2, fs, "positive drift", "GBM_02.png", ylim=(0, 70))
    print("positive drift done", flush=True)
    _panel(-0.2, fs, "negative drift", "GBM_03.png")
