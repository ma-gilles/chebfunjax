...
Translation of opt/OptimInt.m by Nick Trefethen
...
def run():
    ...
    fig.tight_layout()
    _savefig(fig, os.path.join(_IMG, "OptimInt_01.png"))
    r2 = np.asarray((Ia - 0.25).roots())
    print("r =")
    for v in r2:
        print(f"  {v:>18.15f}")
    ax.plot(r2, np.asarray(Ia(r2)), '.k', ms=10)          # hold on
    _savefig(fig, os.path.join(_IMG, "OptimInt_02.png"))
    plt.close(fig)
