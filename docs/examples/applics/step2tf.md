...
<!-- Source: https://www.chebfun.org/examples/applics/Step2tf.html -->

<div class="chebfun-import">
<div class='page-header'>
<h1>The AAA algorithm for system identification (2)</h1>
<h2>Stefano Costa, December 2021 in <a href='../'>applics</a><span><a href='/examples/applics/Step2tf.m'>download</a><span class='sep-sm'>·</span><a href='//github.com/chebfun/examples/blob/master/applics/Step2tf.m'>view on GitHub</a></span></h2>
</div>

<div id='content' class="col-sm-12 example" role="main">
<p>The AAA algorithm, the FFT, and Vandermonde with Arnoldi can be effectively employed together for modelling LTI (Linear Time-Invariant) systems from their response to standard input signals, typically step functions, which is a common practice in engineering. This example is a companion, and complement, to the previous "The AAA algorithm for system identification".</p>
<pre class="mcode-input">tic</pre>

<p>The following is a LTI system transfer function in the Laplace domain, featuring an undamped oscillating component: $$ G(s) = \frac{(1+105s)(1+\frac{1.4}{0.05}s+ \frac{1}{0.05^2}s^2)}{(1+100s)(1+\frac{1.4}{0.04}s+\frac{1}{0.04^2}s^2)(1+0.4s^2)}. $$</p>
...
Stp = @(s) 1./s;          % L-transform of unit step</pre>

<p>Here are the numerical conditions:</p>
...
<p>We compute the step response in the Laplace domain and AAA-approximate the result to find the poles; samples are mirrored in order to enforce symmetry. Hence the signal in the time domain: $$ {\mathcal{L}}^{-1} \left( \sum_n\frac{c_n}{s-a_n}\right) = \sum_n c_n\,e^{a_nt}. $$ Note that the effect of the dominant conjugate poles is almost completely obfuscated by the undamped oscillations due to the purely complex pair.</p>
...
legend('step response of G(s)',LO,SE)</pre>

<pre class="mcode-output">polG =
...
[~,k] = min(abs(polH)); polH(k) = 0             % fix pole of step input</pre>

<pre class="mcode-output">fft_length =
  int16
   1281
polH =
...
<p>Here's a better idea: let's do LS directly on the original signal, and exploit all available data to recompute residues!</p>
...
xlabel('frequency [rad/s]'), ylabel('magnitude')</pre>

<pre class="mcode-output">resH =
...
<p>We pollute the signal with normally distributed noise with a standard deviation of $10^{-2}$, and drop a random 15% of samples.</p>
...
<p>The original, uncorrupted signal is decently restored, and the LTI system identified.</p>
