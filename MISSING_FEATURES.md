...
| ~~26~~ | ~~chebfun-level blowup/singular exponents~~ **MOSTLY CLOSED (Fable 5)**: the SingFun class is now wired into the chebfun factory. `chebfun(f, domain, exps=...)` (single + multi-interval, MATLAB per-breakpoint / per-interval / NaN-autodetect parsing), `blowup=1/2` with `singType` autodetection (pole & fractional finders), `isinf`/`isfinite`/`isnan` honouring endpoint exponents, `sqrt`/root-power emitting branch-point SingFun pieces, SingFun `innerProduct`/`fliplr`/`vscale`, and the `tan`-with-poles constructor.  Flipped: constructor_singfun (explicit-exps + all blowup/singType autodetect cases), tan (#568), sqrt root-touching, isinf/isfinite singular, all singular, fliplr singular.  **Remaining:** cell-array-of-operators construction (`{op1,op2}`), interior-singularity splitting (`splitting`+singular: constructor pass 13/16/17), unbounded-domain SingFun blow-up, and a fracDiff SingFun-valued result (fracDiff of x still ~1.95e-8).  Two tight-tolerance cases kept skipped with measured error (singular innerProduct 1.16e-11 vs 9.84e-12; NaN-exps mixed build ~1.4e-11) |
