"""Port of MATLAB Chebfun tests/trigtech/test_max.m (Opus 4.8[1m]).

max(f) returns the maximum value and its location on [-1, 1].
...
def _spotcheck(fn, exact_max):
    f = _tt(fn)
    y, x = f.max()
    fx = jnp.asarray(fn(jnp.atleast_1d(x))).ravel()[0]
    vs = f.vscale
    return (abs(complex(np.asarray(y)) - exact_max) < 100 * vs * EPS
            and abs(complex(np.asarray(fx)) - exact_max) < 100 * vs * EPS)
...
class TestTrigtechMax:
    ...
    def test_complex_valued(self):
        assert _spotcheck(
            lambda x: 1 + jnp.cos(jnp.pi * x) + jnp.exp(1j * jnp.pi * x), 3)
