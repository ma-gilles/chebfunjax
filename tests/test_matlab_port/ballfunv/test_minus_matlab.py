...
class TestBallfunvMinus:
    def test_minus(self):
        v = _v()
        w = v - v
        assert float(jnp.max(jnp.abs(w.components[0](RR, LL, TT)))) < 1e-13
        assert float(jnp.max(jnp.abs(w.components[1](RR, LL, TT)))) < 1e-13
        assert float(jnp.max(jnp.abs(w.components[2](RR, LL, TT)))) < 1e-13
