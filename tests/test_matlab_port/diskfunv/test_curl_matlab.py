...
class TestDiskfunvCurl:
    def test_all_matlab_assertions(self):
        assert Diskfunv.empty().curl().isempty()  # pass(1)
        F = Diskfunv(_dsk(lambda x, y: jnp.cos(x)),
                     _dsk(lambda x, y: jnp.sin(y)))
        ident = (F.components[1].diffx()
                 - F.components[0].diffy())
        assert _norm_inf(ident - F.curl()) < 1e5 * TOL  # pass(3)
        z = _dsk(lambda x, y: 0 * x)
        assert _norm_inf(Diskfunv(z, z).curl()) < TOL  # pass(4)
        f = _dsk(lambda x, y: jnp.cos(x + y ** 2)
                 + jnp.sin(y) + y)
        assert _norm_inf(f.gradient().curl()) < 1e6 * TOL  # 5
