...
- scalar-tech + row-vector "scalar expansion" (pass 23) is unsupported (src gap).

Array-valued: Chebtech now supports (n, m) coefficient matrices, so the
array-valued plus cases (pass 12:17, 22) are ported.  Chebtech1 still drops
the imaginary part of a complex *scalar* addend, so the array-valued exact
check with the complex ALPHA (pass 15) xfails on Chebtech1 just like pass 3.
...
class TestChebtechPlus:
    ...
    def test_array_valued_zero_sum_commute(self, Tech):
    ...
    def test_array_valued_zero_sum_exact(self, Tech):
    ...
    def test_array_valued_plus_scalar_commute(self, Tech):
    ...
    def test_array_valued_sum_commute(self, Tech):
    ...
    def test_array_valued_sum_exact(self, Tech):
    ...
    def test_array_valued_scalar_row_expansion(self, Tech):
        ...
        g_exact = jnp.stack(
            [1 + jnp.sin(X), 2 + jnp.cos(X), 3 + jnp.exp(X)], axis=-1
        )
        assert _ninf(g(X) - g_exact) < 10 * g.vscale * EPS

    def test_scalar_expansion_in_chebtech(self, Tech):
        # pass(n, 23): sin(x) + [1 2 3] (scalar expanded to array-valued).
        pytest.skip(
            "scalar-valued Chebtech + row-vector [1 2 3] scalar-expansion is "
            "unsupported: __add__ cannot broadcast an (m,) array against a "
            "scalar-column (n,) coeff array — src gap, reported"
        )
