...
class TestChebfun3Max:
    """max(cos(xyz)) over any single dim is the constant 1."""

    def setup_method(self):
        self.f = Chebfun3.from_function(lambda x, y, z: jnp.cos(x * y * z))

    def _err_vs_one(self, h):
        return float(np.max(np.abs(np.asarray(h(_JA, _JB)) - 1.0)))

    def test_default(self):
        assert self._err_vs_one(self.f.max()) < TOL

    def test_g_none(self):
        assert self._err_vs_one(self.f.max(None)) < TOL

    def test_dim2(self):
        assert self._err_vs_one(self.f.max(None, 2)) < TOL

    def test_dim3(self):
        assert self._err_vs_one(self.f.max(None, 3)) < TOL

    def test_dim4_returns_f(self):
        assert self.f.max(None, 4) is self.f
